...
#5.2秒のタイマーが終わると、ローカルのMMDagent.exeと二つのリモートのMMDagent.exeへ音声合成コマンドを発信
...
def createLog():
  ...
  filename+='.txt'
  f =open('C:/Users/KAKU/Documents/GitHub/multiple-agent/logs'+filename,"w")
  return f

def getFormatedNowtime():
  now = int(time.time())
  timeArray = time.localtime(now)
...
def print_and_save(str,f):
    now = int(time.time())
    timeArray = time.localtime(now)
    otherStyleTime = time.strftime("%H:%M:%S  ", timeArray)
    print(otherStyleTime,str)
    f.write(otherStyleTime+str+'\n')
...
#create log file
  ...
  address1 = (global_name.remoteIP1, 39390)
  address2=(global_name.remoteIP2, 39390)
  ...
  mmdSocket2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
  mmdSocket2.connect(address2)
  mmdSocket2.setblocking(0)
  ...
  #recv from 另一台PC to 获取麦克风状态
    ...
    try:
      log2 = mmdSocket2.recv(512)
      #print('agent2 log2:',log2)
      log2=log2.decode('sjis')

    except:
      #print(' ')
      a=1
    else:
      if 'SYNTH_EVENT_STOP' in log2:
        AI2speaking=False
...
#判断说话状态
    if data0>1500:#阈值
      if NSspeaking!=True:
        print_and_save('local说话',f)
      NSspeaking=True
    else:
      if NSspeaking!=False:
        print_and_save('local沉默',f)
      NSspeaking=False

    if data1>1500:#阈值
      if NNSspeaking!=True:
        print_and_save('remote说话',f)
      NNSspeaking=True
    else:
      if NNSspeaking!=False:
        print_and_save('remote沉默',f)
      NNSspeaking=False




    if NSspeaking==True or NNSspeaking==True or AI0speaking==True or AI1speaking==True or AI2speaking==True:
      silenceStartTime=-1
      continue
      ...
      if silenceStartTime==-1: #检测到沉默且此时未在计时（即沉默的起始点）
        ...
        print_and_save('沉默区间开始',f)
      #沉默，且Timer已经on
      elif silenceStartTime!=-1 and time.time()-silenceStartTime>=Turn_Switching_Pause: #Timer启动中
        ...
        if select==0:
          ...
          AI0speaking=True
          print_and_save('agent0 介入',f)
        elif select==1:
          mmdSocket1.send((synthCMD+resp+'\n').encode('sjis'))
          #mmdSocket1.send((motionCMD+'\n').encode('sjis'))
          AI1speaking=True
          print_and_save('agent1 介入',f)
        elif select==2:
          mmdSocket2.send((synthCMD+resp+'\n').encode('sjis'))
          #mmdSocket2.send((motionCMD+'\n').encode('sjis'))
          AI2speaking=True
          print_and_save('agent2 介入',f)
